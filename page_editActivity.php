					...
					<div class="col-md-10 col-md-offset-1">
						...
						if (isset($_GET["activity_id"]) && ($_GET["status"]) == "success") {
							...
							<div class="alert alert-success alert-dismissible">
								...
								แก้ไขข้อมูลกิจกรรม <b><?php echo($row_activity["activity_name"]); ?></b> เรียบร้อยแล้ว
							</div>
							<?php
						}
						?>
						...
						<form action="page_editActivity_save.php" method="POST">
									...
									<div class="row">
										<div class="col-md-12">
											<div class="form-group">
												...
												<input type="text" class="form-control" name="activity_id" pattern="[0-9]{1,}" title="โปรดระบุข้อมูลเป็นตัวเลข 0-9 เท่านั้น" minlength="11" maxlength="11" style="background: #eee;" required="" <?php echo "value='" . $row_activity["activity_id"] . "'"; ?> readonly>
											</div>
											<div class="form-group">
												<label>ชื่อกิจกรรม</label>
												<input type="text" class="form-control" name="activity_name" required="" <?php echo "value='" . $row_activity["activity_name"] . "'"; ?>>
											</div>
											...
											<div class="form-group">
												...
												<select class="form-control" name="activity_type" required="">
													...
													while ($row_activity_type = $activity_type->fetch()) {
														echo "<option value='" . $row_activity_type["type_id"] . "'" . ($row_activity["activity_type"] == $row_activity_type["type_id"] ? 'selected=""':'') . ">" . $row_activity_type["type_name"] . "</option>";
													}
													?>
												</select>
											</div>
											...
											<div class="form-group">
												...
												<select class="form-control" name="activity_organizer" required="">
													...
													while ($row_activity_organizer = $activity_organizer->fetch()) {
														echo "<option value='" . $row_activity_organizer["organizer_id"] . "'" . ($row_activity["activity_organizer"] == $row_activity_organizer["organizer_id"] ? 'selected=""':'') . ">" . $row_activity_organizer["organizer_name"] . "</option>";
													}
													?>
												</select>
											</div>
											...
											<div class="form-group">
												...
												<input type="text" class="form-control" name="activity_unit" pattern="[0-9]{1,}" title="โปรดระบุข้อมูลเป็นตัวเลข 0-9 เท่านั้น" minlength="1" maxlength="2" required="" <?php echo "value='" . $row_activity["activity_unit"] . "'"; ?>>
											</div>
											...
											<div class="form-group">
												...
												<div class="input-group form-group date">
													...
													<input type="text" class="form-control pull-right" id="datepicker" name="activity_date" required="" <?php echo "value='" . revert_pattern($row_activity["activity_date"]) . "'"; ?>>
												</div>
											</div>
											<div class="form-group">
												<label>สถานะกิจกรรม</label>
												<select class="form-control" name="activity_status" required="">
													<option value="1" <?php echo($row_activity["activity_status"] == "1" ? 'selected=""':'') ?>>เปิด</option>
													<option value="0" <?php echo($row_activity["activity_status"] == "0" ? 'selected=""':'') ?>>ปิด</option>
												</select>
